...
namespace ISDAOC_Web.Designer.Controllers
{
    ...
    public class DataModelsController : Controller
    {
        ...
        public DataModelsController(StructureDBContext context,
        ...
        [HttpPost]
        public IActionResult CreateDataModel([FromBody]CRUDModel<DCAppDataModelViewModel> value)
        {
            var data = value.Value;
            
            data.GroupId =Guid.Parse(value.Params["GroupId"].ToString());
        ...
        #region DataFieldChoices

        [HttpPost]
        public IActionResult GetDataFieldChoices([FromBody] CRUDModel<DataManagerRequest> model)
        {
            //model.Principal = User;
            string dataFieldId = model.Params["DataFieldId"].ToString();
            var data = model.Value;
                       
            var serviceMessage = _dataModelService.GetDataFieldChoiceItems(new DCAppDataFieldViewModel() { Id= dataFieldId });
            if (serviceMessage.Result == DCAppServiceResult.suceeded)
            {
                IEnumerable<DCAppDataChoiceItemViewModel> DataSource = ((IEnumerable<DCAppDataChoiceItemViewModel>)serviceMessage.Data).ToList();

                if (DataSource.ToList().Count > 0 && data!=null)
                {
                    DataOperations operation = new DataOperations();
                    if (data.Sorted != null && data.Sorted.Count > 0) //Sorting
                    {
                        DataSource = operation.PerformSorting(DataSource, data.Sorted);
                    }
                    if (data.Where != null && data.Where.Count > 0) //Filtering
                    {
                        DataSource = operation.PerformFiltering(DataSource, data.Where, data.Where[0].Operator);
                    }
                }
        ...
        public PartialViewResult GetDataFieldChoiceData([FromBody] NodeData model)
        {
            //var dataViewModel = new DataModelAssistanceViewModel
            //{
            //    DataModelId = Guid.Parse("41117abd-801c-4ddf-8395-431e0f358c89"/*model.NodeId*/),
            ...
            //DCAppDataFieldChoice dataField = _dBContext.DAppDataFieldChoices.Include(y => y.DCAppDataModel).FirstOrDefault(x => x.Id.ToString() == model.NodeId);
            //DCAppDataChoiceItemViewModel dataFieldViewModel = DataFieldChoiceEntityBuilder.BuildViewModel(dataField);
            //, dataFieldViewModel
            return PartialView("_DataFieldChoiceData");
        }

        [HttpPost]
        public IActionResult DialogUpdateDataFieldChoice([FromBody] CRUDModel<DCAppDataChoiceItemViewModel> value)
        {
            return PartialView("_DialogUpdateDataFieldChoice", value.Value);
        }

        [HttpPost]
        public IActionResult DialogCreateDataFieldChoice([FromBody] CRUDModel<DCAppDataChoiceItemViewModel> value)
        {
            return PartialView("_DialogCreateDataFieldChoice");
        }

        [HttpPost]
        public IActionResult CreateDataFieldChoice([FromBody] CRUDModel<DCAppDataChoiceItemViewModel> value)
        {
            string dataFieldId = value.Params["DataFieldId"].ToString();
            var data = value.Value;
            
            data.DataFieldId = dataFieldId;
            var message = _dataModelService.CreateDataFieldChoiceItem(data);
            if (message.Result == DCAppServiceResult.suceeded)
            {
                var newData = (DCAppDataChoiceItemViewModel)message.Data;
                data.Id = newData.Id;
            }
        ...
        [HttpPost]
        public IActionResult UpdateDataFieldChoice([FromBody] CRUDModel<DCAppDataChoiceItemViewModel> value)
        {
            string dataFieldId = value.Params["DataFieldId"].ToString();
            var data = value.Value;
            
            data.DataFieldId = dataFieldId;
            var message = _dataModelService.UpdateDataFieldChoiceItem(data);
            if (message.Result == DCAppServiceResult.suceeded)
            {
                var newData = (DCAppDataChoiceItemViewModel)message.Data;
                data.Id = newData.Id;
        ...
        [HttpPost]
        public void RemoveDataFieldChoice([FromBody] CRUDModel<DCAppDataChoiceItemViewModel> value)
        {
            string dataFieldId = value.Params["DataFieldId"].ToString();
            var data = new DCAppDataChoiceItemViewModel(new DCAppDataChoiceItem(Guid.Parse(value.Key.ToString())));
            data.DataFieldId = dataFieldId;
            var message = _dataModelService.RemoveDataFieldChoiceItem(data);
            if (message.Result == DCAppServiceResult.suceeded)
            {
            }
        }
